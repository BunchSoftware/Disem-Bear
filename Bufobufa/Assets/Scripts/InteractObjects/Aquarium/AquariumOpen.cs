...
public class AquariumOpen : MonoBehaviour
{
    ...
    [SerializeField] DialogManager Dialog; //УДАЛИТЬ
    public string CodeWord = ""; //УДАЛИТЬ
    ...
    private void Start()
    {
        Dialog = GameObject.Find("DialogManager").GetComponent<DialogManager>(); //УДАЛИТЬ
    ...
    private void Update()
    {
        ...
        {
            Dialog.RunConditionSkip(CodeWord); //УДАЛИТЬ
            OffStrela.SetActive(false); //УДАЛИТЬ
            OnStrela.SetActive(true); //УДАЛИТЬ
